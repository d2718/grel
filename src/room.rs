...
impl Room {
    ...
    */
    ...
    /** Add the given user ID to the list of `User`s who are "banned"
    (prohibited from entering) the `Room`.
    
    This also removes the given user ID from the list of "invited"
    (see `.invite()`, below) `User`s, if present.
    */
